...
function Button({ children, href = "#", variant = "primary", icon = "arrow" }) {
  const styles = {
    primary:
      "bg-rotary-gold text-rotary-ink hover:bg-white shadow-lg shadow-yellow-900/10",
    secondary:
      "bg-white text-rotary-blue hover:bg-rotary-mist shadow-lg shadow-blue-900/10",
    outline:
      "border border-white/55 text-white hover:bg-white hover:text-rotary-blue",
    blue: "bg-rotary-blue text-white hover:bg-rotary-deep shadow-lg shadow-blue-900/15",
  };
  return e(
    "a",
    {
...
function SectionHeading({ eyebrow, title, copy, align = "left" }) {
  return e(
    ...
    e("p", { className: "mb-3 text-sm font-extrabold uppercase tracking-[0.24em] text-rotary-blue" }, eyebrow),
    ...
    e(
      ...
      e(
        ...
        values.map(([title, copy, icon], index) =>
          e(Reveal, { key: title, delay: index * 0.05 }, e("article", { className: "h-full rounded-2xl border border-slate-100 bg-white p-6 shadow-soft" }, Icon({ name: icon, className: "mb-5 h-8 w-8 text-rotary-blue" }), e("h3", { className: "text-xl font-black" }, title), e("p", { className: "mt-3 leading-7 text-slate-600" }, copy)))
        )
      ),
      e(
        "div",
        ...
        e("ol", { className: "grid gap-4" }, ["Charter launch and member onboarding", "First community needs assessment", "Signature education and health projects", "District partnerships and annual impact report"].map((item, i) => e("li", { key: item, className: "flex gap-4 rounded-2xl border border-slate-100 p-5 shadow-sm" }, e("span", { className: "grid h-10 w-10 shrink-0 place-items-center rounded-full bg-rotary-gold font-black text-rotary-ink" }, `0${i + 1}`), e("span", { className: "self-center font-bold text-slate-700" }, item))))
      )
    )
  );
}
        ...
        people.map(([name, role, bio], i) =>
          e(
            ...
            e(
              ...
              e("img", {
                className: "mx-auto h-28 w-28 rounded-full object-cover ring-4 ring-rotary-gold/40",
    ...
    e(
      ...
      e(
        ...
        stats.map(([label, value]) =>
          e(
            "div",
            { key: label, className: "rounded-2xl bg-rotary-blue p-6 text-white" },
...
function Events() {
  ...
  return e(
    ...
    e("div", { className: "mx-auto max-w-7xl" }, e(SectionHeading, { eyebrow: "Events & meetings", title: "Easy ways to show up, serve, and belong.", copy: "Meeting badges, RSVP actions, and calendar-style cards are optimized for mobile taps." }), e("div", { className: "grid gap-5 lg:grid-cols-3" }, events.map(([day, month, title, badge, time]) => e("article", { key: title, className: "flex gap-4 rounded-2xl bg-white p-5 shadow-soft" }, e("div", { className: "grid h-20 w-20 shrink-0 place-items-center rounded-2xl bg-rotary-blue text-center text-white" }, e("span", { className: "text-3xl font-black" }, day), e("span", { className: "text-xs font-bold uppercase" }, month)), e("div", null, e("span", { className: "rounded-full bg-rotary-gold/20 px-3 py-1 text-xs font-black" }, badge), e("h3", { className: "mt-3 text-lg font-black" }, title), e("p", { className: "mt-1 text-sm font-bold text-slate-500" }, time), e("a", { href: "#membership", className: "mt-4 inline-flex rounded-full bg-rotary-blue px-4 py-2 text-sm font-black text-white" }, "RSVP"))))))
  );
}
...
function ScrollTop() {
  ...
  return e("button", { className: `fixed bottom-5 right-5 z-40 grid h-12 w-12 place-items-center rounded-full bg-rotary-gold text-rotary-ink shadow-soft transition ${show ? "opacity-100" : "pointer-events-none opacity-0"}`, onClick: () => window.scrollTo({ top: 0, behavior: "smooth" }), "aria-label": "Scroll to top" }, Icon({ name: "top" }));
}
